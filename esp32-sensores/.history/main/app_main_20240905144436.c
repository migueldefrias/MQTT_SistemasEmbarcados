  ...
  while(true)
  {
    if(xQueueReceive(filaDeInterrupcao, &pino, portMAX_DELAY))
    {
      ...
      int estado = gpio_get_level(pino);
        ...
        case botaoVidros:
            int statusVidros = 1;
            snprintf(attributes, sizeof(attributes), "{\"farol\": %d, \"botaoVidros\": %d, \"botaoTravar\": %d, \"travadoStatus\": %d}", farolStatus,statusVidros, botaoTravarStatus, travadoStatus);
            mqtt_envia_mensagem("v1/devices/me/attributes", attributes);

            sprintf(botaoVidrosStr, "%d", statusVidros);
...
int separaParametros(const char *json_str, char *method) {
 ...
 void processaMetodo(const char *data, char *method) {    
     ...
     if(strcmp(method, "travadoStatus")==0){
        ...
        sprintf(attributes, "{\"vidros\": %d}", vidroStatus);
        mqtt_envia_mensagem("v1/devices/me/attributes", attributes);
        sprintf(statusVidros, "%d", 0);
        mqtt_envia_mensagem2("sensores/vidros", statusVidros);
    }
     if(strcmp(method, "luzInterna")==0){
        printf("Luz Interna %d\n", valor);
        int luzInternaPwm = valor;
        sprintf(attributes, "{\"luzInterna\": %d}", luzInternaPwm);
        mqtt_envia_mensagem("v1/devices/me/attributes", attributes);
        char pwm[10];
        sprintf(pwm, "%d", luzInternaPwm);
        mqtt_envia_mensagem2("sensores/luzInterna", pwm);
    }
    else {
