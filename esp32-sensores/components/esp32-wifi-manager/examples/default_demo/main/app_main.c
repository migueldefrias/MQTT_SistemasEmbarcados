...
#include <esp_netif.h>
#include "esp_log.h"
#include <inttypes.h> // Inclua o cabeçalho para as macros PRI
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            } else if (event->client == client2) {
                requestID2 = event2->event_id;
                printf("TOPIC=%.*s\r\n", event2->topic_len, event2->topic);
                printf("Event ID: %d\n", requestID2);
                printf("DATA=%.*s\r\n", event2->data_len, event2->data);
                processaMetodo(event2->data, method);
            }
            break;
