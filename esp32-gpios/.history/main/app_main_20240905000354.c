...
void processaMetodo(const char *data, char *method, char *topic ) {
    int valor = separaParametros(data, method);
    //printf("Method: %s\n", method);
    if(strcmp(topic, "sensores/farol")){
        farolStatus = valor;
        farois(valor)
        sprintf(farolStatusStr, "%d", farolStatus);
        snprintf(attributes, sizeof(attributes), "{\"porta\": %d, \"luzInterna\": %d, \"travadoStatus\", %d, \"farol\", %d}", portaStatus, luzInternaStatus, travadoStatus, farolStatus);
    ...
    else if(strcmp(topic, "sensores/vidros")){
        descerVidros() 
    ...
    else if(strcmp(topic,"sensores/trava"){
        if(travadoStatus == 0){
            travar();
            travadoStatus = 1;
        }
        ...
        snprintf(attributes, sizeof(attributes), "{\"porta\": %d, \"luzInterna\": %d, \"travadoStatus\", %d, \"farol\", %d}", portaStatus, luzInternaStatus, travadoStatus, farolStatus);
        mqtt_envia_mensagem("v1/devices/me/attributes", attributes);
        mqtt_envia_mensagem2("sensores/travadoStatus", travadoStatusStr);
    })
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            processaMetodo(event->data, method,event->topic );
