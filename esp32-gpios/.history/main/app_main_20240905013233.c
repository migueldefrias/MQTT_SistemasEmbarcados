...
void processaMetodo(const char *data, char *method, char *topic, int valorTopico) {
    int valor = separaParametros(data, method);
    //printf("Method: %s\n", method);
    printf("Topic:%s\n", topic);
    printf("ValorTopico: %d\n", valorTopico);
    printf("Valor: %d\n", valor);
    //printf("TOPIC=%.*s\r\n", topic_len, topic);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            char valor[5];
            sprintf(valor, "%.*s\r", event->data_len, event->data);
            processaMetodo(event->data, method, topic, valor);
