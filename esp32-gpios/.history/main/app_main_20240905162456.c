...
void processaMetodo(const char *data, char *method, char *topic, char *valorTopico) {
    ...
    else if(strcmp(topic,"sensores/travadoStatus")==0){
        printf("Entrou no trava\n");
        //printf("Valor Topico: %c\n", )
       if(strcmp(valorTopico,"1")==0){
            travar();
            travadoStatus = 1;
        }else if (travadoStatus == 1){
            printf("Destravado\n");
            destravar();
            travadoStatus = 0;
    ...
    else if(strcmp(topic, "sensores/luzInterna")==0){
        valor = atoi(valorTopico);
        //sprintf(valor, "%s", valorTopico);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            esp_mqtt_client_subscribe(client2, "sensores/travadoStatus" , 0);
            esp_mqtt_client_subscribe(client2, "sensores/luzInterna", 0);
            esp_mqtt_client_subscribe(client2, "sensores/temperatura", 0);
            esp_mqtt_client_subscribe(client2, "sensores/umidade", 0);
            esp_mqtt_client_subscribe(client2, "sensores/luminosidade", 0);
...
void dispararAlarme(){
    ...
    while(travadoStatus ==1){
        gpio_set_level(buzina, 1);
        vTaskDelay(1000 / portTICK_PERIOD_MS);
        gpio_set_level(buzina, 0);
        vTaskDelay(1000 / portTICK_PERIOD_MS);
    }
}
void configuraSleep()
{
    ...
    gpio_wakeup_enable(botaoBiometria, GPIO_INTR_HIGH_LEVEL);
    //gpio_wakeup_enable(botaoPorta, GPIO_INTR_HIGH_LEVEL);
  ...
  while(true)
  {
    ...
    else if(rtc_gpio_get_level(botaoBiometria)==1){
           printf("Acionado\n");
           printf("Travar Status: %d\n", travadoStatus);
           if(travadoStatus == 1){
                destravar();
                travadoStatus = 0;
                //snprintf(attributes, sizeof(attributes), "{\"porta\": %d, \"luzInterna\": %d, \"travadoStatus\", %d, \"farol\", %d}", portaStatus, luzInternaStatus, travadoStatus, farolStatus);
    ...
    // else if(rtc_gpio_get_level(botaoPorta)==1){
    //     if(travadoStatus == 1){
    //         xTaskCreate(dispararAlarme, "alarmeDisparado", configMINIMAL_STACK_SIZE, NULL, 5, NULL);  
    //     }
    // }
...
void app_main(void)
{
    ...
    xTaskCreate(configuraSleep, "configuraSleep", 2048, NULL, 5, NULL); //thread de light sleep
