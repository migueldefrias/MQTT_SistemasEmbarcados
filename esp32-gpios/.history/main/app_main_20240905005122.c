...
int separaParametros(const char *json_str, char *method, char *topic, char *topic_len) {
...
void processaMetodo(const char *data, char *method, char *topic, char *topic_len){
    int valor = separaParametros(data, method, topic, topic_len);
    //printf("Method: %s\n", method);
    printf("Topic: %s\n", topic);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            processaMetodo(event->data, method,event->topic, event->topic_len);
